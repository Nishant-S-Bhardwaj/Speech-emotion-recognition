...
def extract_features(file_name):
    try:
        ...
        if pitch.size == 0:
            ...
            pitch_scaled = np.mean(pitch)
        
        # Extract LPC
        lpc_coeffs = librosa.lpc(audio, order=13)
        if lpc_coeffs.size == 0:
            lpc_coeffs = np.zeros(13)
...
# Test the prediction on a new file
...
# Visualizing the RMS values for each emotion in two separate graphs
def plot_rms_values(rms_values, labels, test_rms=None):
    ...
    for i, emotion in enumerate(emotion_classes):
        ...
        for idx, rms in enumerate(emotion_rms):
            axs[i].plot(rms, label=f'{emotion.capitalize()} RMS' if idx == 0 else "", color='C'+str(i))
