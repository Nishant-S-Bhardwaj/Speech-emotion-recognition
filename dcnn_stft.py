"""
Speech Emotion Recognition using STFT features and a Deep Convolutional Neural Network (DCNN).

This script extracts STFT, RMS, and pitch features from audio files, trains a DCNN, and evaluates its performance.
"""
...
def extract_stft(file_path: str, n_fft: int = 2048, hop_length: int = 512) -> np.ndarray:
    y, sr = librosa.load(file_path, sr=None)
    stft = np.abs(librosa.stft(y, n_fft=n_fft, hop_length=hop_length))
    return stft.T

def pad_features(features: List[np.ndarray], max_len: int = 100, is_2d: bool = True) -> np.ndarray:
    padded_features = []
    for feature in features:
        if feature.shape[0] < max_len:
            pad_width = max_len - feature.shape[0]
            if is_2d:
                padded_feature = np.pad(feature, ((0, pad_width), (0, 0)), 'constant')
            else:
                padded_feature = np.pad(feature, (0, pad_width), 'constant')
        else:
            padded_feature = feature[:max_len] if not is_2d else feature[:max_len, :]
...
def load_dataset(data_folder: str, max_len: int = 100) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    for file in os.listdir(data_folder):
        if file.endswith('.wav'):
            ...
            label = file.split(' ')[0]
            feature = extract_stft(file_path)
            ...
            labels.append(label)
    features_padded = pad_features(features, max_len=max_len, is_2d=True)
    rms_padded = pad_features(rms_values, max_len=max_len, is_2d=False)
    pitch_padded = pad_features(pitch_values, max_len=max_len, is_2d=False)
...
learning_rate_scheduler = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=10, min_lr=1e-6)
...
def predict(model: tf.keras.Model, file_path: str, max_len: int = 100) -> Tuple[str, float]:
    ...
    feature = extract_stft(file_path)
    feature_padded = pad_features([feature], max_len=max_len, is_2d=True)
...
logging.info(f'Confidence level: {confidence_level:.2f}')
...
def plot_history(history, rms_values, pitch_values) -> None:
...
plot_history(history, rms_values, pitch_values)
